...
-(void) setupWithPoints: (NSArray*) points 
{
    ...
    Point2 cpoints[MAXPOINTS];
    NSValue* v;
    int i = 0;
    
    CGPoint min,max;
    
    min = max = [[points objectAtIndex: 0] CGPointValue];
    for( v in points)
    {
        CGPoint cp=[v CGPointValue];
        cpoints[i].x = cp.x;
        cpoints[i].y = cp.y;
        max.x = MAX(cp.x, max.x);
        max.y = MAX(cp.y, max.y);
        
        min.x = MIN(cp.x, max.x);
        min.y = MIN(cp.y, max.y);
        i++;
    }
    
    CGFloat extent = MAX( max.x - min.x, max.y- min.y);
    CGFloat err = extent/100.0;
...
void drawFunc(void* ctx, const CGPathElement *element)
{
    ...
    {
        CGPoint p = CGPointApplyAffineTransform(element->points[0], info->xform);
        CGContextMoveToPoint(info->context, p.x, p.y);
        
    }
    else if (element->type == kCGPathElementAddCurveToPoint)
    {
        CGPoint c1 = CGPointApplyAffineTransform(element->points[0], info->xform);
        CGPoint c2 = CGPointApplyAffineTransform(element->points[1], info->xform);
        CGPoint p = CGPointApplyAffineTransform(element->points[2], info->xform);
        CGContextAddCurveToPoint(info->context, c1.x, c1.y, c2.x, c2.y, p.x, p.y);
    }
}
...
void annoFunc(void* ctx, const CGPathElement *element)
{
    _DrawInfo* info = (_DrawInfo*)ctx;
    
    CGContextSetFillColor(info->context, red);
    if (element->type==kCGPathElementMoveToPoint)
    {
        CGPoint p = CGPointApplyAffineTransform(element->points[0], info->xform);
        CGContextFillRect(info->context, CGRectMake(p.x-1, p.y-1, 3,3));
    ...
    else if (element->type == kCGPathElementAddCurveToPoint)
    {
        ...
        CGContextSetFillColor(info->context, blue);
        CGContextFillRect(info->context, CGRectMake(c1.x-1, c1.y-1, 3,3));
        CGContextFillRect(info->context, CGRectMake(c2.x-1, c2.y-1, 3,3));
...
{
   // nnDebugLog(@"(%f,%f)--(%f,%f),(%f,%f)--(%f,%f)",b[0].x,b[0].y, b[1].x, b[1].y, b[2].x,b[2].y, b[3].x, b[3].y);
    
    if( CGPathIsEmpty(cgPath))
    {
        CGPathMoveToPoint(cgPath,NULL, b[0].x, b[0].y);
    }
    CGPathAddCurveToPoint(cgPath,NULL,b[1].x, b[1].y, b[2].x, b[2].y, b[3].x, b[3].y);
}
 ...
 */
void FitCurve(d, nPts, error, cgPath)
...
CGPathRef cgPath;
{
    ...
    FitCubic(d, 0, nPts - 1, tHat1, tHat2, error, cgPath);
}
 ...
 */
static void FitCubic(d, first, last, tHat1, tHat2, error, cgPath)
Point2	*d;			/*  Array of digitized points */
...
CGMutablePathRef cgPath;
{
    ...
    if (nPts == 2) {
        ...
        AddCurveToPath(3, bezCurve, cgPath);
        free((void *)bezCurve);
    ...
    FitCubic(d, first, splitPoint, tHat1, tHatCenter, error, cgPath);
    V2Negate(&tHatCenter);
    FitCubic(d, splitPoint, last, tHatCenter, tHat2, error, cgPath);
}
